...
func NewHandler(s *service, verbose bool, staticDir string) http.Handler {
	...
	boost := handle(func(c *client) error {
		id, _ := mux.Vars(c.r)["id"]
		rid := c.r.FormValue("retweeted_by_id")
	...
	unboost := handle(func(c *client) error {
		id, _ := mux.Vars(c.r)["id"]
		rid := c.r.FormValue("retweeted_by_id")
	...
	r.HandleFunc("/boost/{id}", boost).Methods(http.MethodPost)
	r.HandleFunc("/unboost/{id}", unboost).Methods(http.MethodPost)
